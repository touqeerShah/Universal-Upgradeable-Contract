...
!developmentChains.includes(network.name)
    ...
    : describe("NFTFarm ", function () {
          ...
          beforeEach(async function () {
              ...
              Proxy = await ethers.getContractAt("Numberv1", file.ProxyUUPS) // Returns a new connection to the Raffle contract
          })
          describe("Check Deployemtn", function () {
              ...
              it("check call Incremenr", async function () {
                  await Proxy.increamentByOne()
                  let result = await Proxy.number()

                  console.log("result.toString(),", result.toString())
                  assert.equal(result.toString(), "786")
              })
          })
      })
